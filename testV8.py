...
Your job is to carefully decompose the user’s request into a complete, executable list of tasks.

**Workflow**

1. Internally reason through the entire request:
   • Identify every concrete sub‑step required to finish the job end‑to‑end.  
   • Ensure no prerequisite, dependency, or deliverable is omitted.  
   • Do *not* ask the user follow‑up questions; make reasonable assumptions instead.

2. Output a JSON object in exactly the following format—nothing more, nothing less:
...
- If the request is naturally single‑step, return a single task in the list.  
- If the request contains multiple distinct steps, stages, or deliverables, return each as a separate task—in the order they should be executed.  
- Keep each task description concise but unambiguous; include explicit deliverables when useful (e.g., “…and provide download link”).  
- Do *not* add explanations, comments, markdown, or any text outside the JSON block.  
- Never ask the user additional questions—assume whatever is reasonable and proceed.

**Examples**

_Simple request_  
User: “Analyze the uploaded sales data and generate a report.”  
Assistant response:  
{
  "tasks": [
    "Analyze the uploaded sales data and generate a report."
  ]
}

_Multi‑step request_  
User: “Analyze the uploaded sales data, generate a dashboard, and summarize the findings in a markdown report.”  
Assistant response:  
{
  "tasks": [
    "Analyze the uploaded sales data and extract key metrics.",
    "Generate a dashboard visualizing the key findings.",
    "Summarize the findings in a markdown report and provide download link."
  ]
}
...
def process_stream(stream, session_id):
    for event in stream:
        if event.event == "thread.message.created":
            ...
            # 立即查询 run 状态，确保 run 是 completed 才结束
            run = client.beta.threads.runs.retrieve(thread_id=thread_mapping[session_id],
            ...
            if run.status == "completed":
                ...
                #yield json.dumps({
                #    "type": "text",
                #    "content": f"Assistant is waiting for further action. Current run status: {run.status}"
                #}) + "\n"
                # 继续监听流
...
def generate(session_id, tasks, file_ids):
    yield json.dumps({"type": "text", "content": f"\n\n💡The following will be handled in several steps:\n\n"}) + "\n"
    for idx, task in enumerate(tasks):
        yield json.dumps({"type": "text", "content": f"\n\n⬜Step: {idx + 1}, task: {task}\n\n"}) + "\n"

    for task in tasks:
        yield json.dumps({"type": "text", "content": f"\n\n⬜TASK: {task} START:\n\n"}) + "\n"
        contents = [{"type": "text", "text": task}]
        attachments = []
        # 判断文件类型
        ...
        with client.beta.threads.runs.stream(
            ...
            for event in process_stream(stream, session_id):
                yield event
                try:
                    event_obj = json.loads(event)
                    if event_obj.get("type") == "done":
                        # 结束任务
                        yield json.dumps({"type": "text", "content": f"\n\n✅TASK: {task} FINISHED.\n\n"}) + "\n"
                        break  # 当前任务结束
                except:
...
def ask():
    ...
    # 任务规划
    planner_history = chat_history + [{"role": "user", "content": question}]
    ...
    try:
        tasks_obj = json.loads(response_text)
        tasks = tasks_obj["tasks"]
        if not isinstance(tasks, list):
            return jsonify({"error": "Planner response is invalid, no tasks list found."}), 400
    except Exception as e:
        print(f"[ASK] JSON parse failed: {e}")
        return jsonify({"error": f"Failed to parse task list: {e}"}), 400

    return Response(generate(session_id, tasks, file_ids), content_type='text/event-stream')
